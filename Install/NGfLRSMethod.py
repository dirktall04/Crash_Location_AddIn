...
# DAT            2015-07-15 -- Changed the ConflateKDOT function's spatialtolerance from "20 feet" to "19 feet"
#                as a workaround for a bug that shows up when using "20 feet".
#-------------------------------------------------------------------------------
...
def InitalizeCurrentPathSettings():
    ...
    class pathInformationClass:
        def __init__(self):
            ...
            self.lyr = lyr
            self.Alias = Alias
            self.RouteNameUnmatched = 0
            self.RouteNameNull = 0
            self.countVariableAbove34 = 0
...
def Kdot_RouteNameCalc(cps):
    ...
    # Rebuilt with regex. Too many problems just trying to slice the strings.
    for keyName in searchDict:
        ...
        if testResult0 is not None and testResult1 is not None:
            ...
            # Did not find any RouteName matches.
            cps.RouteNameUnmatched += 1
            fullRouteName = firstPart + secondPart
        ...
        for keyName in searchDict:
            ...
            if uCursorItem[0] == listItem[0]:
                ...
                if listItem[2] is not None and listItem[2] != "":
                    ...
                    # RouteNameFull is None/Null/Empty.
                    cps.RouteNameNull += 1
            else:
                pass
        ...
        for outRowIndex, outRow in enumerate(outRowContainer):
            ...
            if outRowIndex == 0:
                ...
                if countVariable in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
                    ...
                    # Maximum count variable exceeded.
                    cps.countVariableAbove34 += 1
                    # 
...
def printIssuesReport(cps):
    if (cps.RouteNameUnmatched + cps.RouteNameUnmatched + cps.countVariableAbove34) > 0:
        print "\n########################## Issues Report ##################################"
        if cps.RouteNameUnmatched == 0:
            pass
        else:
            print "Could not find a RouteName match for " + str(cps.RouteNameUnmatched) + " rows."
        
        if cps.RouteNameNull == 0:
            pass
        else:
            print "The RouteName was None/Null/Empty for " + str(cps.RouteNameNull) + " rows."
            
        if cps.countVariableAbove34 == 0:
            pass
        else:
            print "Ran out of Unique Values to use for " + str(cps.countVariableAbove34) + " rows."
        print "###########################################################################\n"
    else:
        pass


def CalledUpon(CalledWithGDB, CalculateInPlace):
    ...
    if CalculateInPlace == False:
        oGDBPath = currentPathSettings.gdbPath
        
        print "oGDBPath = " + str(oGDBPath)
        
        rcGDBPath = oGDBPath[:-4] + "_RoadChecks.gdb"
        
        print "rcGDBPath = " + str(rcGDBPath)
        if Exists(rcGDBPath):
            Delete_management(rcGDBPath)
        Copy_management(oGDBPath, rcGDBPath) # Creates a copy of the original gdb
    else:
        # The copy is already created by the calling function, so use the CalledWithGDB
        # without modifying the path, copying or renaming the GDB.
        oGDBPath = currentPathSettings.gdbPath
        ...
        if rcGDBPath[-15:].lower() == "_roadchecks.gdb":
            print "Warning: CalculateInPlace is: " + str(CalculateInPlace)
            print "Will run NGfLRSMethod modifications on this GDB: " + str(rcGDBPath) + "."
        else:
            print "rcGDBPath does not point to a properly named roadchecks gdb."
            print "Will not run the NGfLRSMethod modifications on it."
    ...
    MakeFeatureLayer_management(rcGDBPath+"\\"+fdNG+"\\RoadCenterline","RoadCenterline","#","#","#")
    TableView(rcGDBPath+"\\RoadAlias", "RoadAlias","#","#","#")
    
    try:
        DisableEditorTracking_management(rcGDBPath+"\\"+fdNG+"\\RoadCenterline" ,"DISABLE_CREATOR","DISABLE_CREATION_DATE","DISABLE_LAST_EDITOR","DISABLE_LAST_EDIT_DATE")
        DisableEditorTracking_management(rcGDBPath+"\\RoadAlias" ,"DISABLE_CREATOR","DISABLE_CREATION_DATE","DISABLE_LAST_EDITOR","DISABLE_LAST_EDIT_DATE")
    except:
    ...
    currentPathSettings = AliasCalc(currentPathSettings)

    HighwayCalc(currentPathSettings)

    #StreetNetworkCheck(currentPathSettings)

    currentPathSettings = createUniqueIdentifier(currentPathSettings)

    #LRS_Tester(currentPathSettings)

    #ReturnStreetstoTopology(currentPathSettings)
    
    printIssuesReport(currentPathSettings)
